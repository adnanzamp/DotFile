...
if [ -f "$ALIASES_FILE" ]; then
    ...
    echo "  Git (core): g, ga, gaa, gc, gco, gcb, gcm, gd, gds, gl, gp, gpl, gs, gst"
    echo "  Git (shortcuts): gk, gr, gri, grc, gra, grs, gm, gma, gcp, gf, gfa, gfp"
    echo "  Git (branch): gb, gba, gbd, gbD, grh, grhh, gclean"
    echo "  Git (stash): gstash, gstashp, gstashl, gstashd"
    echo "  Git (workflow): gwip, gunwip, gpf, gpr, gpro, gprom, glog, glogp"
    echo "  Lazygit: lg"
    echo "  Docker: d, dc, dps, dpsa, di, dex"
    echo "  Development: py, pip, node, npm, yarn"
    echo "  System: c, h, j, path, now, nowtime, nowdate"
    echo "  Network: myip, localip, ports, listening, ping, speedtest, flushdns"
    echo "  Network: netcheck, openports, connections, tcpdump"
